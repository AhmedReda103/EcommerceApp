...
import com.example.ecommerceapp.ApplicationClass
...
class UserAccountViewModel @Inject constructor(
    ...
    private fun saveUserInformationWithNewImage(user: User, imageUri: Uri) {
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(getApplication<ApplicationClass>().contentResolver , imageUri)
                val byteArrayOutputStream = ByteArrayOutputStream()
                imageBitmap.compress(Bitmap.CompressFormat.JPEG , 96 , byteArrayOutputStream)
                val imageByteArray =byteArrayOutputStream.toByteArray()
                val imageDirectory = storageReference.child("profileImages/${firebaseAuth.uid}/${UUID.randomUUID()}")
