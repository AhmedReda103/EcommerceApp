...
class DetailsViewModel @Inject constructor(
            ...
            .whereEqualTo("product.id", cartProduct.product.id).get().addOnSuccessListener {
                 ...
                 it.documents.let {
                     if(it.isEmpty()){//add new product
                         ...
                         if(product == cartProduct){ //update quantity
                             val documentId = it.first().id
                             increaseQuantity(documentId , product)
    ...
    private fun addNewProduct(product: CartProduct){
        firebaseCommon.addProductToCart(cartProduct = product){ addedProduct , e ->
            viewModelScope.launch {
                if(e==null)
